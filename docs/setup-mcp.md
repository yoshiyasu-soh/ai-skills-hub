...
既知の問題が複数あります。** 本手順ではそれらの回避策(Cloudflare API での直接設定)も
含めて記載しています。
...
3つ目の「`mcp` タイプ」アプリは隠れた存在で、これが今回の問題の温床でした(詳細後述)。
...
## 既知の問題と回避策(ベータ版・要Cloudflare API操作)
...
- Cloudflare側でベータ機能の問題(「既知の問題と回避策」参照)が修正された場合、
